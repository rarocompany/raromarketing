...
class WPBakeryShortCode_ts_image extends WPBakeryShortCode {
	...
	protected function content( $atts, $content = null ) {
		...
		ob_start();
        $template = locate_template( 'vc_templates/brabus/image.php');
        if( $template ) {
            include( $template );
        }
        else {
            ?>
            <figure class="image-box <?php echo esc_attr($wrapper_class); ?>" <?php if ($animate_block == 'yes' && $animation_delay != '') {
...
vc_map( array(
	...
	'params' => array(
		array(
			"type" 			=> 	"attach_image",
			"heading" 		=> 	__( 'Background Image', 'themezinho' ),
			"param_name" 	=> 	"image",
			"group" 		=> "General",
		),
		array(
